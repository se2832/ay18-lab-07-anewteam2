...
public class StockQuoteAnalyzerTests {
    ...
    @Test(dataProvider = "normalOperationDataProvider")
    public void testGetSymbolShouldReturnCorrectWhenCalled(StockQuote firstReturn, StockQuote secondReturn, int happyMusicCount, int sadMusicCount,
                                                           double percentChange) throws Exception {
        //Arrange
        analyzer = new StockQuoteAnalyzer("F", mockedStockQuoteGenerator, mockedStockTickerAudio);
        //Act
        boolean first = analyzer.getSymbol().equals(firstReturn.getSymbol());
        boolean second = analyzer.getSymbol().equals(secondReturn.getSymbol());
        //Assert
    ...
    @Test(dataProvider = "normalOperationDataProvider")
    public void testGetCurrentQuoteShouldReturnCorrectWhenCalled(StockQuote firstReturn, StockQuote secondReturn, int happyMusicCount, int sadMusicCount,
                                                                 double percentChange) throws Exception {
        //Arrange
        when(mockedStockQuoteGenerator.getCurrentQuote()).thenReturn(firstReturn, secondReturn);
        analyzer = new StockQuoteAnalyzer("F", mockedStockQuoteGenerator, mockedStockTickerAudio);
        //Act
        boolean nullVal = analyzer.getCurrentQuote() == null;
        analyzer.refresh();
        boolean first = analyzer.getCurrentQuote() == firstReturn;
        analyzer.refresh();
        boolean second = analyzer.getCurrentQuote() == secondReturn;
        //Assert
        Assert.assertTrue(nullVal);
        Assert.assertTrue(first);
        Assert.assertTrue(second);
    }
